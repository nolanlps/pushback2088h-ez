...
void left4rushNolan() { //april 22
  pros::delay(20);
    ...
    intakeone(12000);
    chassis.pid_odom_set({{{-6_in, 25_in}, fwd, 90}}, true);
    ...
    chassis.pid_wait_quick_chain();
chassis.pid_swing_set(RIGHT_SWING, 180, 127, 20);
pros::delay(550);
    hood.toggle();
    intakeall(12000);
    pros::delay(750);
...
matchload.toggle();
  ...
  chassis.pid_odom_set({{{-41_in, 20_in}, fwd, 100}});
  chassis.pid_wait_quick_chain();
    hood.toggle();
  chassis.pid_turn_set(180_deg, 90);
 chassis.pid_wait_quick_chain();

  chassis.pid_odom_set({{{-41_in, 37_in}, rev, 70}});
  chassis.pid_wait_quick_chain();
...
void right4rushNolan() { //april 22
  pros::delay(20);
    ...
    chassis.pid_turn_set(-67, 100);
    chassis.pid_wait_quick_chain();

    chassis.pid_drive_set(-16, 127);
    chassis.pid_wait_quick_chain();
chassis.pid_swing_set(LEFT_SWING, 170, 127, 20);
pros::delay(550);
    hood.toggle();
    intakeall(12000);
    pros::delay(750);
  ...
  chassis.pid_odom_set({{{24_in, 20_in}, fwd, 100}});
  chassis.pid_wait_quick_chain();
    hood.toggle();
  chassis.pid_turn_set(180_deg, 90);
 chassis.pid_wait_quick_chain();

  chassis.pid_odom_set({{{24_in, 37_in}, rev, 70}});
  chassis.pid_wait_quick_chain();
...
// ## RIGHT 7 block potentially ##
    ...
    chassis.pid_odom_set({{{32_in, -2_in}, rev, 90}}, true);
    chassis.pid_wait();
...
void right4long3mid() { //april 22
  ...
  pros::delay(20);
    ...
    chassis.pid_odom_set({{{-7_in, 41.5_in, 315_deg}, fwd, 100}}, true);//meowmoewoemowmewo

    chassis.pid_wait(); 

    intakeall(-8000); //wow lyla ur so kawaii
    chassis.pid_drive_set(-2, 70);
    pros::delay(1800);

    chassis.pid_odom_set({{{32_in, 4_in}, rev, 90}}, true);
    intakeall(0);
    chassis.pid_wait();
    matchload.toggle();
    intakeone(12000);
    ...
    chassis.pid_drive_set(14.7_in, 60);
    // chassis.pid_odom_set({{{32_in, -16_in}, fwd, 80}});
    // chassis.pid_wait_until(13.2_in);
    chassis.pid_wait();
    pros::delay(300);

    chassis.pid_odom_set({{{31.5_in, 32_in}, rev, 75}});
    pros::delay(750);
    hoodToggle();
    intakeall(12000);
    pros::delay(1200);
  ...
  chassis.pid_odom_set({{{22_in, 13_in}, fwd, 100}});
  chassis.pid_wait();
      hoodToggle();
 ...
 chassis.pid_wait();
  ...
  chassis.pid_odom_set({{{23.5_in, 36.5_in}, rev, 100}});
...
void left7ballrush() {
  ...
  pros::delay(20);
    ...
    chassis.pid_odom_set({{{-42.5_in, 18_in}, fwd, 90}});
  chassis.pid_wait();
hood.toggle();
  chassis.pid_turn_set(180_deg, 90);
 chassis.pid_wait_quick_chain();
...
void left4long3mid() { //april 22
  ...
  // hoodToggle();
    ...
    chassis.pid_odom_set({{{-35_in, 0_in}, fwd, 90}}, true);

    chassis.pid_wait();
    intakeone(12000);
    chassis.pid_turn_set(177_deg, 80);
    chassis.pid_wait();
  ...
  pros::delay(40);
    ...
    chassis.pid_odom_set({{{-35_in, 26_in}, rev, 70}});
    pros::delay(650);
    hoodToggle();
    intakeall(12000);
    pros::delay(1150);
...
void counterSawp() {
 ...
 chassis.pid_odom_set({{0_in, -35_in}, rev, 90}, true);
  ...
  chassis.pid_odom_set({{22_in, -36_in}, rev, 90}); // changed from 35
  pros::delay(600);
  hoodToggle();
    matchload.toggle();
  intakeall(12000);
...
// chassis.pid_wait_quick_chain(); 
  ...
  chassis.pid_wait();
  chassis.pid_odom_set({{-39.5, -17}, rev, 110});
  chassis.pid_wait_quick_chain();
...
void skills() {
  ...
  chassis.pid_drive_set(13.2_in, 80, true);
  chassis.pid_wait();
  pros::delay(1500);
...
chassis.pid_odom_set({{{0_in, 16_in}, fwd, 75}}, true);
chassis.pid_wait();
